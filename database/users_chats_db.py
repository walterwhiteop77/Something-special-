...
class Database:    
    def __init__(self, uri, database_name):
        ...
        self.delivery_col = self.db.file_deliveries
    ...
    # ── File Delivery Queue ───────────────────────────────────────────────────
    # Records are auto-expired after 24 hours via a MongoDB TTL index.
    # Each record holds everything Bot2 needs to deliver a file without
    # any file info being exposed in Telegram URLs.
    #
    # Single-file record:
    #   { _id: str, user_id: int, type: 'file', file_id: str, grp_id: int, created_at: datetime }
    # All-files record:
    #   { _id: str, user_id: int, type: 'allfiles', file_ids: [str], grp_id: int, created_at: datetime }

    async def ensure_delivery_indexes(self):
        """Create TTL index so stale delivery records expire automatically."""
        await self.delivery_col.create_index(
            'created_at',
    ...
    async def create_file_delivery(
        ...
        """Insert a delivery record and return its short delivery_id."""
        import random, string, datetime as _dt
        delivery_id = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
        doc = {
            '_id': delivery_id,
            'user_id': user_id,
            'grp_id': grp_id,
        ...
        if type_ == 'file':
            ...
            doc['file_ids'] = file_ids or []
        await self.delivery_col.insert_one(doc)
        return delivery_id

    async def get_file_delivery(self, delivery_id: str):
        """Return the delivery record or None if not found / expired."""
        return await self.delivery_col.find_one({'_id': delivery_id})

    async def del_file_delivery(self, delivery_id: str):
        """Remove a delivery record after it has been consumed."""
        await self.delivery_col.delete_one({'_id': delivery_id})
    # ─────────────────────────────────────────────────────────────────────────

    # ── Bot2 token storage ────────────────────────────────────────────────────
    async def set_bot2_token(self, token: str):
        """Save the Bot2 token to the database."""
        await self.botcol.update_one(
            {'_id': 'bot2_config'},
            {'$set': {'token': token}},
            upsert=True
    ...
    async def get_bot2_token(self) -> str:
        """Return the Bot2 token from the database, or '' if not set."""
        doc = await self.botcol.find_one({'_id': 'bot2_config'})
        return doc.get('token', '') if doc else ''

    async def del_bot2_token(self):
        """Remove the Bot2 token from the database."""
        await self.botcol.delete_one({'_id': 'bot2_config'})
    # ─────────────────────────────────────────────────────────────────────────
