...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.answer(url=f"https://t.me/{_file_bot}?start=get_{delivery_id}")
        ...
        settings = await get_settings(query.message.chat.id)
        _file_bot = temp.U_NAME2 or temp.U_NAME
        files_batch = temp.GETALL.get(key, [])
        file_ids = [f.file_id for f in files_batch]
        delivery_id = await db.create_file_delivery(
            user_id=query.from_user.id,
            grp_id=query.message.chat.id,
            type_='allfiles',
        ...
        try:
            await query.answer(url=f"https://telegram.me/{_file_bot}?start=get_{delivery_id}")
            return
        ...
        _fbot = temp.U_NAME2 or temp.U_NAME
        await query.answer(url=f"href='https://telegram.me/{_fbot}?start=file_{query.message.chat.id}_{file.file_id}")
        ...
        try:
            ...
            if btn:
                ...
                return
            # checksub passes kk="file" and file_id="{grp_id}_{actual_file_id}"
            _file_bot = temp.U_NAME2 or temp.U_NAME
            try:
                parts = file_id.split("_", 1)
                grp_id_cs = int(parts[0])
                actual_file_id = parts[1]
                delivery_id = await db.create_file_delivery(
                    user_id=query.from_user.id,
                    grp_id=grp_id_cs,
                    type_='file',
                ...
                await query.answer(url=f"https://t.me/{_file_bot}?start={kk}_{file_id}")
            await query.message.delete()
        except Exception as e:
        ...
        if imdb:
            ...
            if not settings.get('button'):
                ...
                for idx, file in enumerate(files, start=1):
                    cap += f"<b>\n{idx}. <a href='https://telegram.me/{_fbot}?start=file_{message.chat.id}_{file.file_id}'>[{get_size(file.file_size)}] {clean_filename(file.file_name)}\n</a></b>"
        else:
